...
do 
	...
	do 
		...
		do 
			...
			utility.drawings["Sub"] = utility:drawObject("Text", {Outline = true, Text = "ritual", Color = Color3.new(1, 1, 1), Size = 13})
			...
			do 
				Messages = {
					...
					["Ritual"] = {
						"Owned By Ritual",
						"Ritual Ritual Ritual RAHHHHH",
						"Slammed by Ritual",
						"YOU GOT SLAMMED BY Ritual",
						"WHAT YOU CANT BEAT RITUAL?",
						"PRO TIP: BUY RITUAL",
					},
				}
				utx = {
...
local Window = Library:Window({Size = UDim2.new(0,580,0,600)}) do 
	...
	local misc = Window:Page({Name = "Misc", Weapons = true}) do 
		...
		local chatSpam = Movement:Section({Name = "Chat Spam", Size = 330, Side = "Left"}) do
			...
			chatSpam:Dropdown({Name = "Type", Flag = "Chat Spam Type", Options = {"Ritual", "Scottish"}, Default = "Ritual"})
	...
	local Config = Window:Page({Name = "Settings"}) do
		...
		--
		local CurrentList = {};
		if not isfolder("Ritual") then 
			makefolder("Ritual")
		end 
		if not isfolder("Ritual/Configs") then 
			makefolder("Ritual/Configs")
		end 
        writefile("Ritual/Configs/preset.cfg", game:HttpGet("https://raw.githubusercontent.com/f1nobe7650/Lunar/main/preset.cfg"))
        
		local function UpdateConfigList()
			local List = {};
			for idx, file in ipairs(listfiles("Ritual/Configs")) do
				local FileName = file:gsub("Ritual/Configs\\", ""):gsub(".cfg", "")
		...
		Cfgs:Button({Name = "Create", Callback = function()
			...
			if ConfigName == "" or isfile("Ritual/Configs/" .. ConfigName .. ".cfg") then
				return;
			end;
			writefile("Ritual/Configs/" .. ConfigName .. ".cfg", Library:GetConfig());
			UpdateConfigList();
		end})
		Cfgs:Button({Name = "Save", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				writefile("Ritual/Configs/" .. SelectedConfig .. ".cfg", Library:GetConfig())
			end;
		end})
		Cfgs:Button({Name = "Load", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				Library:LoadConfig(readfile("Ritual/Configs/" .. SelectedConfig .. ".cfg"))
			end
		end})
		Cfgs:Button({Name = "Delete", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				delfile("Ritual/Configs/" .. SelectedConfig .. ".cfg")
			end
			UpdateConfigList();
		end})
...
Library:Notification("Loaded ritual.cc in " .. Floor((tick() - loadingTime) * 1000)  .."ms", 3, Library.Accent, flags["Notification Position"])
